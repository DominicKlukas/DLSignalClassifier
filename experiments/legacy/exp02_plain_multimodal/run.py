...
RESULTS_PATH = BASE_DIR / "experiment2_results.json"
...
class MultimodalIQFFTCNN(nn.Module):
    ...
    def forward(self, iq: torch.Tensor, fft: torch.Tensor) -> torch.Tensor:
        iq_features = self.iq_branch(iq)
        fft_features = self.fft_branch(fft)
        return self.classifier(torch.cat([iq_features, fft_features], dim=1))
    ...
    for iq_batch, fft_batch, y_batch in loader:
        ...
        with torch.set_grad_enabled(training):
            with autocast(device_type=device.type, enabled=use_cuda):
                logits = model(iq_batch, fft_batch)
...
def train_and_evaluate(
    ...
    model = MultimodalIQFFTCNN(num_classes=len(class_names)).to(device)
    ...
    all_preds = []
    with torch.no_grad():
        for iq_batch, fft_batch, _ in test_loader:
            iq_batch = iq_batch.to(device, non_blocking=use_cuda)
            fft_batch = fft_batch.to(device, non_blocking=use_cuda)
            with autocast(device_type=device.type, enabled=use_cuda):
                logits = model(iq_batch, fft_batch)
...
def run_dataset_experiment(dataset_name: str, dataset_path: Path, normalize_rms: bool):
    ...
    print(f"\n=== {dataset_name}: IQ+FFT multimodal CNN ===")
    result = train_and_evaluate(*splits, class_names)
    return {"multimodal_cnn": result, "class_names": class_names}
